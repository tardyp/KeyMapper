...
class KeymapDetectionDelegateTest {
    ...
    fun `don't initialise repeating if repeat when trigger is released after failed long press`() =
        mCoroutineScope.runBlockingTest {
            ...
            //then
            assertThat(mPerformActionTest.history.size, `is`(1))
            assert(mPerformActionTest.history.all { it.action == action1 })
            ...
            //then
            assertThat(mPerformActionTest.history.size, greaterThan(1))
            assert(mPerformActionTest.history.all { it.action == action2 }) {
    ...
    fun `don't initialise repeating if repeat when trigger is released after failed failed double press`() =
        mCoroutineScope.runBlockingTest {
            ...
            //then
            assertThat(mPerformActionTest.history.map { it.action }, `is`(listOf(action2)))
        }
    ...
    fun `don't initialise repeating if repeat when trigger is released after failed double press and failed long press`() =
        mCoroutineScope.runBlockingTest {
            ...
            //then
            assertThat(mPerformActionTest.history.map { it.action }, `is`(listOf(action3)))
        }
    ...
    fun `initialise repeating if repeat until pressed again on failed long press`() =
        mCoroutineScope.runBlockingTest {
            ...
            assertThat(mPerformActionTest.history.size, greaterThan(2))
            assert(mPerformActionTest.history.all { it.action == action1 })
    ...
    fun `initialise repeating if repeat until pressed again on failed double press and failed long press`() =
        mCoroutineScope.runBlockingTest {
            ...
            //then
            mockParallelTriggerKeys(*trigger1.keys.toTypedArray()) //press the key again to stop it repeating
            advanceUntilIdle()
            ...
            assertThat(mPerformActionTest.history.size, greaterThan(1))
            assert(mPerformActionTest.history.all { it.action == action2 }) {
                "not all actions are the action for the long press trigger. actual actions: ${mPerformActionTest.history}"
            }
