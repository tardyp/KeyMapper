...
class KeyMapControllerTest {
    ...
    @TestCaseName("{0}")
    fun invalidInput_downNotConsumed(description: String, keyMap: KeyMap) =coroutineScope.runBlockingTest{
        //GIVEN
        keyMapListFlow.value = listOf(keyMap)
        ...
        keyMap.trigger.keys.forEach {
            val consumed =
                inputKeyEvent(999, KeyEvent.ACTION_DOWN, triggerKeyDeviceToDescriptor(it.device))
    ...
    @Parameters(method = "params_allTriggerKeyCombinations")
    @TestCaseName("{0}")
    fun validInput_downConsumed(description: String, keyMap: KeyMap)=coroutineScope.runBlockingTest {
        //GIVEN
        keyMapListFlow.value = listOf(keyMap)
    ...
    @TestCaseName("{0}")
    fun validInput_dontConsumeFlag_dontConsumeDown(description: String, keyMap: KeyMap) =coroutineScope.runBlockingTest{
        keyMapListFlow.value = listOf(keyMap)
