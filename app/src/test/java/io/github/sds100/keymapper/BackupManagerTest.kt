...
class BackupManagerTest {
    ...
    private lateinit var mockUseCase: io.github.sds100.keymapper.domain.usecases.BackupRestoreUseCase
    ...
    fun init() {
        ...
        mockUseCase =
            Mockito.mock(io.github.sds100.keymapper.domain.usecases.BackupRestoreUseCase::class.java)
        ...
        Mockito.`when`(mockKeymapRepository.requestAutomaticBackup).thenReturn(LiveEvent())
        Mockito.`when`(mockFingerprintMapRepository.requestAutomaticBackup).thenReturn(LiveEvent())
    ...
    fun `backup all fingerprint maps, don't save keymap db version and show success message`() =
        coroutineScope.runBlockingTest {
            ...
            MatcherAssert.assertThat(
                eventStream.history,
                Is.`is`(listOf(BackupResult(Success(Unit))))
            )
        }
    ...
    fun `backup key maps, return list of default key maps, keymap db version should be current database version`() =
        coroutineScope.runBlockingTest {
            ...
            MatcherAssert.assertThat(
                ...
                Is.`is`(AppDatabase.DATABASE_VERSION)
            )
