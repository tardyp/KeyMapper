 ...
 */
class ActiveEdgeActionOptionsViewModel : BaseOptionsDialogViewModel<ActiveEdgeActionOptions>() {

    override val stateKey = "state_active_edge_action_options"

    override fun createSliderListItemModel(option: IntOption): SliderListItemModel =
        when (option.id) {

            ActiveEdgeActionOptions.ID_MULTIPLIER -> {
                SliderListItemModel(
                    id = option.id,
            ...
            ActiveEdgeActionOptions.ID_DELAY_BEFORE_NEXT_ACTION -> {
                SliderListItemModel(
                    id = option.id,
            ...
            ActiveEdgeActionOptions.ID_REPEAT_RATE -> {
                SliderListItemModel(
                    id = option.id,
            ...
            ActiveEdgeActionOptions.ID_HOLD_DOWN_DURATION -> {
                SliderListItemModel(
                    id = option.id,
    ...
    override fun createCheckboxListItemModel(option: BoolOption): CheckBoxListItemModel =
        when (option.id) {
            ActiveEdgeActionOptions.ID_SHOW_PERFORMING_ACTION_TOAST -> {
            ...
            ActiveEdgeActionOptions.ID_SHOW_VOLUME_UI -> {
                CheckBoxListItemModel(
                    id = option.id,
            ...
            ActiveEdgeActionOptions.ID_REPEAT_UNTIL_SWIPED_AGAIN -> {
                CheckBoxListItemModel(
                    id = option.id,
            ...
            ActiveEdgeActionOptions.ID_HOLD_DOWN_UNTIL_SWIPED_AGAIN -> {
                CheckBoxListItemModel(
                    id = option.id,
    ...
    class Factory : ViewModelProvider.NewInstanceFactory() {
        ...
        override fun <T : ViewModel?> create(modelClass: Class<T>): T {
            return ActiveEdgeActionOptionsViewModel() as T
        }
    }
}