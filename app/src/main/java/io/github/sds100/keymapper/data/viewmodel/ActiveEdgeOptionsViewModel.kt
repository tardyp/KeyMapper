...
class ActiveEdgeOptionsViewModel : BaseOptionsViewModel<ActiveEdgeOptions>() {
    companion object {
        private const val STATE_KEY = "state_active_edge_behavior"
    }

    override val stateKey = STATE_KEY

    override fun createSliderListItemModel(option: IntOption) = when (option.id) {
        ActiveEdgeOptions.ID_VIBRATION_DURATION -> {
            SliderListItemModel(
                id = option.id,
    ...
    override fun createCheckboxListItemModel(option: BoolOption) = when (option.id) {
        ActiveEdgeOptions.ID_VIBRATE -> CheckBoxListItemModel(
            id = option.id,
    ...
    class Factory : ViewModelProvider.NewInstanceFactory() {
        ...
        override fun <T : ViewModel?> create(modelClass: Class<T>): T {
            return ActiveEdgeOptionsViewModel() as T
        }
    }
}